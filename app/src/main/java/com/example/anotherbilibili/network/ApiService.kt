...
interface ApiService {
    ...
    @GET("https://bangumi.bilibili.com/appindex/follow_index_fall")
    fun getInformationData(
        @Query("appkey") appkey: String, @Query("build") build: String, @Query("cursor") cursor: Int, @Query("mobi_app") mobi_app: String, @Query(
            "platform"
        ) platform: String, @Query("ts") ts: String, @Query("sign") sign: String
    ): Observable<InformationData>
